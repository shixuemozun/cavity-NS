...
double p_[56][56],p1[56][56],pg[56][56]; 
...
int main()
{
	...
	{
	cout<<"循环次数：" << t <<"  " <<"进度：" << (double)t/600000<<endl;
	...
	for(j=1;j<55;j++)
	{
		...
		ru_[54][j] = 0;
		//ru_[54][j] = 2*ru_[53][j]-ru_[52][j]; //边界条件外推
		ru[54][j] = ru_[54][j];
		u[54][j] = ru[54][j]/rho;
...
//利用更新的 p值重新计算 u、v; 
	...
	ostringstream name;
	  ...
	  out<< "Title= \"LBM Lid Driven Flow\"\n" << "VARIABLES=\"X\",\"Y\",\"u\",\"v\"\n" << "ZONE T=\"BOX\",I=" << 55 << ",J=" << 55 << ",F=POINT" << endl;
	  for(int j=0;j<=54;j++)
	     for(int i=0;i<=54;i++)
	     {
	     	
			 
			 out<<i*dx<<" "<<j*dx<<" "<<u[i][j]<<" "<<v[i][j]<<endl;
